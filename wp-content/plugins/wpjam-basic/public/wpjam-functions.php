...
function wpjam_validate_path_item($item, $path_types){
	...
	if(is_wp_error($result)){
		if(count($path_types) > 1 && $result->get_error_code() == 'invalid_page_key'){
...
function wpjam_option_get_setting($name, $setting_name='', $default=null){
	$object = wpjam_get_option_object($name);

	return $object ? $object->get_setting($setting_name, $default) : $default;
}

function wpjam_option_update_setting($name, $setting_name, $value){
	$object = wpjam_get_option_object($name);

	return $object ? $object->update_setting($setting_name, $value) : null;
}

function wpjam_option_delete_setting($name, $setting_name){
	$object = wpjam_get_option_object($name);

	return $object ? $object->delete_setting($setting_name) : null;
}
...
function wpjam_get_post_images($post=null, $large='large', $thumbnail='thumbnail'){
	$object	= wpjam_get_post_object($post);
...
function wpjam_parse_query($wp_query, $args=[], $parse_for_json=true){
	$object	= new WPJAM_Query_Parser($wp_query, $args);

	return $parse_for_json ? $object->parse($args) : $object->render($args);
}
...
function wpjam_get_related_posts($post=null, $args=[], $parse_for_json=false){
	...
	return wpjam_parse_query($wp_query, $args, $parse_for_json);
}
...
function wpjam_register_bind($name, $appid, $args){
	$object	= wpjam_get_bind_object($name, $appid);

	if($object){
		return $object;
	}

	$object	= WPJAM_Bind::create($name, $appid, $args);

	return WPJAM_Bind::register($object);
}

function wpjam_get_bind_object($name, $appid){
	return WPJAM_Bind::get($name.':'.$appid);
}

// User Signup
function wpjam_register_user_signup($name, $args){
	return WPJAM_User_Signup_Type::register($name, $args);
}
...
function wpjam_generate_verification_code($key, $group='default'){
	...
	return new WP_Error('invalid_verification_code_group', '无效的验证码分组');
}
