...
class WPJAM_List_Table extends WP_List_Table{
	public function __construct($args=[]){
		...
		if(method_exists($args['model'], 'get_actions')){
			$args['actions']	= $this->get_actions_by_model();
	...
	public function __get($name){
		...
		}elseif($name == 'meta_type'){
			if($this->data_type == 'post_type'){
				return 'post';
			}elseif($this->data_type == 'taxonomy'){
				return 'term';
			}else{
				$meta_type	= $this->get_meta_type_by_model();

				if($meta_type && !is_wp_error($meta_type)){
					return $meta_type;
				}
			}
		}
	...
	public function parse_args($args){
		...
		if($this->data_type == 'post_type' && $this->post_type){
			foreach(WPJAM_Post_Option::get_by_post_type($this->post_type) as $object){
				$object->register_list_table_action();
			}
		}elseif($this->data_type == 'taxonomy' && $this->taxonomy){
			foreach(WPJAM_Term_Option::get_by_taxonomy($this->taxonomy) as $object){
				$object->register_list_table_action();
			}
		}
		...
		$so_object		= WPJAM_Screen_Option::get_instance('wpjam_list_table');
		$screen_option	= $so_object->get_option([]);
		$screen_option	= wp_parse_args($screen_option, [
			'ajax'		=> true, 
			'form_id'	=> 'list_table_form',
		...
		if($this->sortable){
			...
			$screen_option['sortable']	= $sortable;
		...
		$so_object->set_option($screen_option);
			...
			if($field['type'] == 'fieldset' && wpjam_array_get($field, 'fieldset_type') != 'array'){
				foreach($field['fields'] as $sub_key => $sub_field){
					if(!empty($sub_field['show_admin_column'])){
						$sub_field	= wpjam_array_except($sub_field, ['data_type', 'post_type', 'taxonomy']);
		...
		$objects	= WPJAM_List_Table_Action::get_registereds();
		
		foreach(wpjam_list_sort($objects) as $key => $object){
			if(!$this->is_available($object)){
				continue;
			}

			$object->list_table_object	= $this;

			if(!$object->bulk_callback && method_exists($this->model, 'bulk_'.$object->name)){
				$object->bulk_callback	= [$this->model, 'bulk_'.$object->name];
			}

			if(!$object->value_callback && method_exists($this->model, 'value_callback')){
			...
			if(method_exists($this->model, $method)){
				...
				if($this->meta_type){
					if($fields = $this->get_fields($list_action, $id, ['include_prev'=>true])){
						$defaults	= wpjam_get_fields_defaults($fields);
					}else{
						$defaults	= null;
					}
...
class WPJAM_List_Table_Setting extends WPJAM_Register{
	public static function get_object($name){
		...
		if(method_exists($object->model, 'admin_head')){
			add_action('admin_head',	[$object->model, 'admin_head']);
		}

		if(method_exists($object->model, 'admin_footer')){
			add_action('admin_footer',	[$object->model, 'admin_footer']);
		}
...
class WPJAM_List_Table_Action extends WPJAM_Register{
	public function get_fields($id){
		...
		if(!$fields){
			$fields	= $this->list_table_object->get_fields_by_model($this->name, $id) ?: [];
		}
	...
	public function get_data($id, $fields=[]){
		...
		if($data_callback && is_callable($data_callback) && $fields){
			...
			$data	= $this->list_table_object->get_by_model($id);
	...
	public function get_submit_text($id, $name=null){
		if($name){
			...
			return array_map(function($item){ 
				...
				return wp_parse_args($item, ['response'=>$this->response, 'class'=>'primary']);
			}, $submit_text);
		}
	}
	...
	public function callback($id=0, $data=null, $submit_name='', &$response_type=''){
		...
		if($bulk){
			...
			if($callback){
				...
				return $this->list_table_object->call_model_list_action($id, $data, $this->name);
			}
		}
	}
	...
	public function get_row_action($args=[]){
		...
		if(($args['layout'] == 'calendar' && !$this->calendar) || !$this->show_if($args['id'])){
			return '';
		}
