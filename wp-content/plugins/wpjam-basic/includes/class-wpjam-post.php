...
class WPJAM_Post{
	...
	public function get_images($large_size='large', $thumbnail_size='thumbnail'){
		$images	= [];

		if($this->images){
			$sizes	= [];

			$sizes['large']	= $large_size ?: ($this->get_type_setting('large_size') ?: 'large');

			if($thumbnail_size !== false){
				$sizes['thumbnail']	= $thumbnail_size ?: ($this->get_type_setting('thumbnail_size') ?: 'thumbnail');
			}
	...
	public function parse_for_json($args=[]){
		...
		if($this->supports('images')){
			$images_size	= wpjam_array_get($args, ['images_size']);

			$json['images']	= $this->get_images($thumbnail_size, $images_size);
		}
...
class WPJAM_Post_Type extends WPJAM_Register{
	...
	public function get_keys(){
		return ['thumbnail_size', 'images_size', 'plural'];
	}
	...
	public function get_fields(){
		if(is_null($this->_fields)){
			...
			if(post_type_supports($this->name, 'images')){
				...
				if($this->images_size){
					$fields['images']['size']			= $this->images_size;
					$fields['images']['description']	= '尺寸：'.$this->images_size;
				}
			}
