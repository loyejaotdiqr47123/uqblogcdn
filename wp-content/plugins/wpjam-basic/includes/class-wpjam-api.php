...
class WPJAM_JSON extends WPJAM_Register{
	...
	public static function parse_name($action){
	...
	public static function __callStatic($method, $args){
		if(in_array($method, ['parse_post_list_module', 'parse_post_get_module'])){
			...
			return (new WPJAM_JSON_Module('post_type', array_merge($args, ['action'=>$action])))->parse();
		}
	}
}
...
class WPJAM_Grant extends WPJAM_Option_Items{
	...
	public function __call($method, $args){
		if(str_starts_with($method, 'cache_')){
			...
			$cg_obj	= WPJAM_Cache_Group::get_instance('wpjam_api_times', ['prefix'=>$today]);
...
class WPJAM_Parameter{
	...
	public static function get_input($name=''){
		if(is_null(self::$input)){
			...
			self::$input	= is_array($input) ? $input : [];
		}
	...
	public static function get_data(){
		if(is_null(self::$data)){
			self::$data	= wpjam_get_parameter('data', ['method'=>'POST', 'sanitize_callback'=>'wp_parse_args', 'default'=>[]]);
			self::$data	= wpjam_array_merge(self::get_defaults(), self::$data);
		}
