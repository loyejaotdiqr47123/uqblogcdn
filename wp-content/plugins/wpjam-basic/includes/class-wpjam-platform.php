		...
		if($item){
			...
			if($callback){
				...
				if(!empty($item['page_type'])){
					$object	=  wpjam_get_data_type_object($item['page_type']);
	...
	public function get_fields(){
		...
		foreach($this->types as $type => $item){
			...
			if($item_fields){
				...
				$object	= !empty($item['page_type']) ? wpjam_get_data_type_object($item['page_type']) : null;
...
class WPJAM_Data_Type extends WPJAM_Register{
	...
	public function parse_json_schema($args){
		return $this->callback('parse_json_schema', $args);
	}

	public function query_items($args){
		return $this->callback('query_items', $args);
	}

	public function get_path($args){
		return $this->callback('get_path', $args) ?: '';
	}

	public function get_field($args){
		return $this->callback('get_field', $args) ?: [];
	}

	public function get_fields($args){
		return $this->callback('get_fields', $args) ?: [];
	}

	public function query_label($value, $args){
		$result	=  $this->callback('query_label', $value, $args);

		return $result ?? $value;
	}

	public function parse($value, $args){
		$result	= $this->callback('parse_value', $value, $args);

		return $result ?? $value;
	}

	public function validate($value, $args){
		$result	= $this->callback('validate_value', $value, $args);
	...
	public static function ajax_query(){
		...
		if($data_type = wpjam_get_parameter('data_type', ['method'=>'POST'])){
			if($object = self::get($data_type)){
...
class WPJAM_Post_Type_Data_Type{
	...
	public static function query_items($args){
		...
		return array_map(function($post){ return ['label'=>$post->post_title, 'value'=>$post->ID]; }, $query->posts);
	}
...
class WPJAM_Taxonomy_Data_Type{
	...
	public static function query_items($args){
		if($terms = get_terms(wp_parse_args($args, [
			'number'		=> 10,
			'hide_empty'	=> 0
		]))){
			return array_map(function($term){ return ['label'=>$term->name, 'value'=>$term->term_id]; }, $terms);
		}
...
class WPJAM_Model_Data_Type{
	...
	public static function query_items($args){
		...
		if($items = $query->items){
			return array_map(function($item)use($label_key, $id_key){ 
