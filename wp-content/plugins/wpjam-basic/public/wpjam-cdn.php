...
Version: 2.0
*/
class WPJAM_CDN extends WPJAM_Register{
	public static function get_sections(){
		$options		= array_merge([''=>' '], wp_list_pluck(self::get_registereds(), 'title'));
		$cdn_fields		= [
	...
	public static function get_setting($name, $default=null){
		return wpjam_option_get_setting('wpjam-cdn', $name, $default);
	}

	public static function update_setting($name, $value){
		return wpjam_option_update_setting('wpjam-cdn', $name, $value);
	}
	...
	public static function on_plugins_loaded(){
		...
		if(CDN_NAME){
			...
			if(!is_admin()){
				...
				foreach(['exts', 'dirs'] as $k){
					if($v = self::get_setting($k)){
						if(!is_array($v)){
							self::update_setting($k, WPJAM_CDN::parse_items($v));
						}
					}
				}
			}
			...
			if(self::get_setting('image', 1)){
				if($object = WPJAM_CDN::get(CDN_NAME)){
					if($object->file && file_exists($object->file)){
						$callback	= include $object->file;
